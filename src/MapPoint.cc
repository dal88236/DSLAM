...
MapPoint::MapPoint():
    ...
    mnLastFrameSeen(0), mnBALocalForKF(0), mnFuseCandidateForKF(0), mnBASPDForKF(0), mbProcessedBySPD(false), mnLoopPointForKF(0), mnCorrectedByKF(0),
    mnCorrectedReference(0), mnBAGlobalForKF(0), mnVisible(1), mnFound(1), mbBad(false), mbMarked(true),
...
MapPoint::MapPoint(const Eigen::Vector3f &Pos, KeyFrame *pRefKF, Map* pMap):
    ...
    mbMarked(true), mpReplaced(static_cast<MapPoint*>(NULL)), mfMinDistance(0), mfMaxDistance(0), mpMap(pMap),
...
MapPoint::MapPoint(const Eigen::Vector3f &Pos, Map* pMap, Frame* pFrame, const int &idxF):
    ...
    mnBALocalForKF(0), mnFuseCandidateForKF(0), mnBASPDForKF(0), mbProcessedBySPD(false), mnLoopPointForKF(0), mnCorrectedByKF(0),
    mnCorrectedReference(0), mnBAGlobalForKF(0), mpRefKF(static_cast<KeyFrame*>(NULL)), mnVisible(1),
    mnFound(1), mbBad(false), mbMarked(true), mpReplaced(NULL), mpMap(pMap), mnOriginMapId(pMap->GetId())
{
    SetWorldPos(Pos);
...
void MapPoint::SetMarked()
{
    unique_lock<mutex> lock(mMutexFeatures);
    mbMarked = true;
}

void MapPoint::SetUnmarked()
{
    unique_lock<mutex> lock(mMutexFeatures);
    mbMarked = false;
}

bool MapPoint::isMarked() 
{
    unique_lock<mutex> lock(mMutexFeatures);
    return mbMarked;
}
